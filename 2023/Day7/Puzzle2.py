...
cardStrength = {'A': 13, 'K': 12, 'Q': 11, 'T': 9, '9': 8, '8': 7, '7': 6, '6': 5, '5': 4, '4': 3, '3': 2, '2': 1, 'J': 0}
...
class Hand:
    ...
    def _getDeckType(self):
        ...
        if "J" in tempCards:
            cardsMinusJ = set(self.cards)
            cardsMinusJ.discard("J")
            if cardsMinusJ:
                highestValCard = sorted(cardsMinusJ, key=lambda x: self.cards.count(x))[-1]
                tempCards = tempCards.replace("J", highestValCard)

        for card in set(tempCards):
            cardCounts[tempCards.count(card)] += 1
